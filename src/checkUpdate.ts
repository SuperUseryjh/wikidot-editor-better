...
import { platformGetScriptVersion, platformNotify, platformOpenInTab, platformRequest } from './platform';

/**
 * 检查脚本更新（在注入器/隔离世界上下文运行）。
 * 定期拉取静态站点的 version.json，发现新版本时通过通知提示，
 * 点击通知后在新标签页打开新版本安装页。
 */
export async function checkUpdate(): Promise<void> {
    const currentScriptVersion = platformGetScriptVersion();
    if (!currentScriptVersion) {
        logError('无法获取当前脚本版本，跳过更新检查。');
        return;
    }
    ...
    const response = await platformRequest({ method: 'GET', url: updateUrl, timeout: 15000 });
    if (!response.ok) {
        logError('检查更新失败:', response.error === 'timeout' ? '请求超时' : `HTTP ${response.status}`);
        return;
    }

    try {
        const remotePackageJson = JSON.parse(response.text);
        ...
        if (comparison > 0) {
            ...
            platformNotify(
                'Wikidot Editor Better 有新版本',
                `当前 ${currentScriptVersion}，最新 ${remoteVersion}。点击通知前往更新。`,
                () => platformOpenInTab(userScriptUrl, false)
            );
